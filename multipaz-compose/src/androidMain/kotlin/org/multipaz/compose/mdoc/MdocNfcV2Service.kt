...
/**
 * Base class for implementing NFC engagement according to ISO/IEC 18013 Second Edition.
 *
...
abstract class MdocNfcV2Service: HostApduService() {
    companion object {
        private val TAG = "MdocNfcV2Service"
    ...
    private fun startEngagement() {
        ...
        engagement = MdocNfcV2EngagementHelper(
            ...
            onHandoverComplete = { connectionMethods, encodedDeviceEngagement, handover ->
                ...
                println("Going hybrid")
                hybridTransport = NfcHybridTransportMdoc(
                    sendMessageViaNfc = { message ->
                        engagement?.sendMessage(message)
                    }
                )
                runBlocking { hybridTransport!!.open(eDeviceKey.publicKey) }
            ...
            onMessageReceived = { message ->
                val t0 = Clock.System.now()
                Logger.iCbor(TAG, "Received payload", message.toByteArray())
                if (hybridTransport != null) {
                    hybridTransport!!.onMessageReceivedViaNfc(message)
                } else {
                    Logger.i(TAG, "No hybrid")
                }
            },
            onError = { error ->
                ...
                vibrateError()
                engagement?.processOnDeactivated()
                engagement = null
            },
